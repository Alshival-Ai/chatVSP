...
export type CriticalAgentType = "kmz" | "design";
export type DeployCriticalAgentStatus = "already_exists" | "restored" | "created";

export interface DeployCriticalAgentResponse {
  agent_name: string;
  status: DeployCriticalAgentStatus;
}
...
export async function deployCriticalAgent(
  agent: CriticalAgentType
): Promise<DeployCriticalAgentResponse> {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorText || "Failed to deploy agent");
  }
