...
import { CriticalAgentType, deployCriticalAgent } from "./lib";
...
function MainContent({
  ...
  refreshPersonas: () => Promise<unknown> | void;
}) {
  const [deployingAgent, setDeployingAgent] = useState<CriticalAgentType | null>(
    null
  );
  ...
  const handleDeployAgent = async (agent: CriticalAgentType) => {
    try {
      setDeployingAgent(agent);
      const response = await deployCriticalAgent(agent);
      if (response.status === "already_exists") {
        toast.warning("Agent already exists!");
      } else {
        toast.success(`${response.agent_name} deployed.`);
        await Promise.resolve(refreshPersonas());
      }
    } catch (error) {
      toast.error(
    ...
    <div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          <CreateButton href="/app/agents/create?admin=true">New Agent</CreateButton>
          <Button
            secondary
            onClick={() => handleDeployAgent("kmz")}
            disabled={deployingAgent !== null}
          >
            {deployingAgent === "kmz" ? "Deploying..." : "Deploy KMZ Agent"}
          </Button>
          <Button
            secondary
            onClick={() => handleDeployAgent("design")}
            disabled={deployingAgent !== null}
          >
            {deployingAgent === "design"
              ? "Deploying..."
              : "Deploy Design Agent"}
          </Button>
        </div>
