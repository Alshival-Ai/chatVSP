...
function extractMapArtifact(data: unknown): MapArtifactData | null {
  const candidate = parseObjectLike(data);
  if (!candidate) {
    return null;
  }

  const nestedToolResult = parseObjectLike(candidate.tool_result);

  const artifact =
    parseObjectLike(candidate.map_artifact) ||
    parseObjectLike(nestedToolResult?.map_artifact);

  if (!artifact) {
    return null;
  }

  const sourceFileId = artifact.source_file_id;
  ...
  return {
    ...
    source_file_name:
      typeof artifact.source_file_name === "string"
        ? artifact.source_file_name
        : undefined,
    feature_count:
      typeof artifact.feature_count === "number"
        ? artifact.feature_count
        : undefined,
    coordinate_point_count:
      typeof artifact.coordinate_point_count === "number"
        ? artifact.coordinate_point_count
        : undefined,
  };
}
...
export const CustomToolRenderer: MessageRenderer<CustomToolPacket, {}> = ({
  ...
  const mapArtifact = useMemo(() => extractMapArtifact(data), [data]);
          ...
          {mapArtifact && (
            <div className="rounded-lg border border-border p-2">
              <div className="mb-2 text-xs text-muted-foreground">
                Map preview
                {typeof mapArtifact.feature_count === "number"
                  ? ` | features: ${mapArtifact.feature_count}`
                  : ""}
                {typeof mapArtifact.coordinate_point_count === "number"
                  ? ` | points: ${mapArtifact.coordinate_point_count}`
                  : ""}
              </div>
              <KmzMapPreview
                fileId={mapArtifact.source_file_id}
                fileName={mapArtifact.source_file_name || "KMZ Output"}
              />
            </div>
          )}
