...
import { CODEX_LABS_PATH } from "@/app/codex-labs/constants";
...
const MemoizedAppSidebarInner = memo(
  ({ folded, onFoldClick }: AppSidebarInnerProps) => {
    ...
    const isCodexLabsEnabledFromSettings =
    ...
    const isCodexLabsEnabled =
      isCodexLabsEnabledFromSettings || user?.enable_codex_labs === true;
      ...
      () => (
        <div data-testid="AppSidebar/codex-labs">
          <SidebarTab
            icon={SvgTerminal}
            folded={folded}
            href={CODEX_LABS_PATH}
            onClick={() => posthog?.capture("clicked_codex_labs_in_sidebar")}
          >
            Codex Labs
          </SidebarTab>
        </div>
