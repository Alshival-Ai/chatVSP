...
const AppInputBar = React.memo(
  ({
    ...
    const isUploadingFiles = chatState === "uploading" || hasUploadingFiles;
    ...
    const handleFileClick = useCallback(
      (file: ProjectFile) => {
        ...
        const documentForViewer: MinimalOnyxDocument = {
          document_id: file.is_chat_file
            ? file.file_id
            : `project_file__${file.file_id}`,
    ...
    return (
      <Disabled disabled={disabled} allowClick>
        ...
        >
          ...
          {isUploadingFiles && (
            <div className="px-3 pb-1 flex items-center gap-1 text-sm text-text-03">
              <SvgLoader className="h-4 w-4 animate-spin" />
              <span>Uploading file{currentMessageFiles.length === 1 ? "" : "s"}...</span>
            </div>
          )}
