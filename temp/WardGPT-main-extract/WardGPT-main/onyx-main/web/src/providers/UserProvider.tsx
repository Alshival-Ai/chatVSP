...
export function UserProvider({
  ...
  const hasWarmedCodexLabsRef = useRef(false);
  ...
  useEffect(() => {
    if (hasWarmedCodexLabsRef.current) {
      return;
    }
    ...
    const codexLabsEnabled =
      updatedSettings?.settings?.codex_labs_enabled === true ||
      upToDateUser?.enable_codex_labs === true;
    if (!codexLabsEnabled) {
      return;
    }

    hasWarmedCodexLabsRef.current = true;
    void fetch("/api/codex-labs/warmup", { method: "POST" }).catch((error) => {
      console.error("Failed to warm Codex Labs session:", error);
      hasWarmedCodexLabsRef.current = false;
    });
  }, [
    upToDateUser?.id,
    upToDateUser?.enable_codex_labs,
    updatedSettings?.settings?.codex_labs_enabled,
  ]);
