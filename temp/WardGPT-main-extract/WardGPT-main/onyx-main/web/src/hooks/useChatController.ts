...
function chatDescriptorToProjectFile(
  file: File,
  ...
  return {
    ...
    chat_file_type: descriptor.type as ChatFileType,
    token_count: null,
    chunk_count: null,
  };
}

function createUploadingProjectFile(file: File): ProjectFile {
  const tempId =
    typeof crypto !== "undefined" && typeof crypto.randomUUID === "function"
      ? `chat_upload_${crypto.randomUUID()}`
      : `chat_upload_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
...
export default function useChatController({
  ...
  const { fetchProjects, setCurrentMessageFiles } = useProjectsContext();
      ...
      try {
        ...
        while (!stack.isComplete || !stack.isEmpty()) {
          ...
          if (
            ...
            stack.isEmpty() &&
            packets.length > 0 &&
            !packets.some((packet) => packet.obj.type === "stop")
          ) {
            const lastPlacement = packets[packets.length - 1]?.placement ?? {
              turn_index: 0,
            };
            // Defensive fallback: close out timeline rendering when the
            // backend stream ends without an explicit STOP packet.
            stack.push({
          ...
          if (!stack.isEmpty() && !controller.signal.aborted) {
            ...
            const extractMessageIds = (
              candidate: unknown
            ): MessageResponseIDInfo | null => {
              if (typeof candidate !== "object" || candidate === null) {
                return null;
              }
              if (
                !Object.hasOwn(candidate, "user_message_id") ||
                !Object.hasOwn(candidate, "reserved_assistant_message_id")
              ) {
                return null;
              }
              const maybeMessageIds = candidate as {
                user_message_id: unknown;
                reserved_assistant_message_id: unknown;
              };
              if (
                typeof maybeMessageIds.user_message_id !== "number" ||
                typeof maybeMessageIds.reserved_assistant_message_id !==
                  "number"
              ) {
                return null;
              }
              return {
                user_message_id: maybeMessageIds.user_message_id,
                reserved_assistant_message_id:
                  maybeMessageIds.reserved_assistant_message_id,
              };
            };

            const packetWithMessageIds =
              extractMessageIds(packet) ||
              (Object.hasOwn(packet, "obj")
                ? extractMessageIds((packet as Packet).obj)
                : null);

            const hasMessageIds = packetWithMessageIds !== null;

            if (packetWithMessageIds !== null) {
              newUserMessageId = packetWithMessageIds.user_message_id;
            ...
            if (packetWithMessageIds !== null) {
              newAgentMessageId =
                packetWithMessageIds.reserved_assistant_message_id;
            }

            if (hasMessageIds) {
              // Initial ID reservation packet. IDs already captured above.
  ...
  const handleMessageSpecificFileUpload = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      const optimisticUploadFiles = acceptedFiles.map(createUploadingProjectFile);
      const optimisticUploadIds = new Set(optimisticUploadFiles.map((f) => f.id));
      setCurrentMessageFiles((prev) => [...prev, ...optimisticUploadFiles]);

      updateChatStateAction(getCurrentSessionId(), "uploading");
      const [uploadedDescriptors, uploadError] = await uploadFilesForChat(
        Array.from(acceptedFiles)
      );
      if (uploadError) {
        setCurrentMessageFiles((prev) =>
          prev.filter((file) => !optimisticUploadIds.has(file.id))
        );
        toast.error(uploadError);
        updateChatStateAction(getCurrentSessionId(), "input");
        return;
      }
      const uploadedMessageFiles = uploadedDescriptors.map((descriptor, index) =>
        chatDescriptorToProjectFile(
          acceptedFiles[index] ?? new File([], descriptor.name ?? "attachment"),
          descriptor
        )
      );
      setCurrentMessageFiles((prev) => [
    ...
    [liveAgent, llmManager, forcedToolIds, setCurrentMessageFiles]
  );
