...
class TestTranslateHistoryToLlmFormat:
    ...
    def test_openai_emits_input_file_for_supported_non_image_attachments(self) -> None:
        history = [
            ChatMessageSimple(
        ...
        assert any(isinstance(p, TextContentPart) for p in translated[0].content)
        file_parts = [p for p in translated[0].content if isinstance(p, FileContentPart)]
        assert len(file_parts) == 1
        assert file_parts[0].filename == "sample.pdf"
        assert file_parts[0].file_data

    def test_non_openai_does_not_emit_input_file_parts(self) -> None:
        history = [
            ChatMessageSimple(
                message="Please summarize this file.",
                token_count=5,
        ...
        assert isinstance(translated[0], UserMessage)
        assert isinstance(translated[0].content, list)
        assert not any(isinstance(p, FileContentPart) for p in translated[0].content)

    def test_openai_skips_unsupported_file_extension(self) -> None:
        history = [
            ChatMessageSimple(
                message="Use this archive.",
                token_count=5,
                message_type=MessageType.USER,
                non_image_files=[
                    ChatLoadedFile(
    ...
    def test_openai_skips_synthetic_kmz_template_input_file(self) -> None:
        history = [
            ChatMessageSimple(
                message="Use the KMZ template.",
                token_count=5,
                message_type=MessageType.USER,
                non_image_files=[
                    ChatLoadedFile(
                        file_id="__kmz_workbook_template__",
                        content=b"fake-xlsx-content",
                        file_type=ChatFileType.DOC,
                        filename="P1_NWF_3.4.26_KMZ_Input_ALL_P1.xlsx",
                        content_text=None,
                        token_count=1,
                    )
                ],
