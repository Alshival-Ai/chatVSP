...
# 2b. Ensure Python venv template has the latest required packages.
# Run on every startup so dependency updates are applied to existing templates.
if [ -d "$VENV_TEMPLATE_PATH" ] && [ -f "$REQUIREMENTS_PATH" ]; then
    echo "  Syncing Python packages in venv template..."
