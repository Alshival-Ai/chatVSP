...
from onyx.db.local_mcp_server import rotate_local_mcp_server_api_key
...
@shared_task(
    name=OnyxCeleryTask.ROTATE_LOCAL_MCP_API_KEY,
    soft_time_limit=JOB_TIMEOUT,
    bind=True,
    base=AbortableTask,
)
def rotate_local_mcp_api_key_task(self: Any, tenant_id: str) -> bool:  # noqa: ARG001
    """Rotate local MCP API key and refresh MCP Action admin credentials."""
    if self.is_aborted():
        raise TaskRevokedError("rotate_local_mcp_api_key_task was aborted.")

    with get_session_with_current_tenant() as db_session:
        changed = rotate_local_mcp_server_api_key(db_session)
        if changed:
            sync_local_mcp_server_tools(db_session)
            task_logger.info(
                "Successfully rotated local MCP API key and auto-synced MCP tools."
            )
        else:
            task_logger.debug(
                "Local MCP API key rotation skipped (feature disabled or server missing)."
            )
        return changed


@shared_task(
    name=OnyxCeleryTask.SYNC_LOCAL_MCP_TOOLS,
    soft_time_limit=JOB_TIMEOUT,
    bind=True,
    base=AbortableTask,
)
def sync_local_mcp_tools_task(self: Any, tenant_id: str) -> bool:  # noqa: ARG001
    """Discover local MCP tools and sync them to DB."""
    if self.is_aborted():
        raise TaskRevokedError("sync_local_mcp_tools_task was aborted.")

    with get_session_with_current_tenant() as db_session:
        changed = sync_local_mcp_server_tools(db_session)
        if changed:
            task_logger.info("Successfully synced local MCP tools.")
        else:
            task_logger.debug(
                "Local MCP tool sync skipped or produced no DB changes."
            )
        return changed
