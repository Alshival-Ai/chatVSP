...
from onyx.configs.app_configs import LOCAL_MCP_API_KEY_ROTATION_INTERVAL_SECONDS
...
from onyx.configs.app_configs import LOCAL_MCP_TOOL_SYNC_INTERVAL_SECONDS
...
# Add local MCP API key rotation task when enabled.
if (
    MCP_SERVER_ENABLED
    and LOCAL_MCP_API_KEY_ROTATION_ENABLED
    and not LOCAL_MCP_STATIC_API_KEY
):
    beat_task_templates.append(
        {
            "name": "rotate-local-mcp-api-key",
            "task": OnyxCeleryTask.ROTATE_LOCAL_MCP_API_KEY,
            "schedule": timedelta(seconds=LOCAL_MCP_API_KEY_ROTATION_INTERVAL_SECONDS),
            "options": {
                "priority": OnyxCeleryPriority.LOW,
                "expires": BEAT_EXPIRES_DEFAULT,
            },
...
# Add local MCP tools sync task when enabled.
if MCP_SERVER_ENABLED and LOCAL_MCP_TOOL_SYNC_ENABLED:
    beat_task_templates.append(
        {
            "name": "sync-local-mcp-tools",
            "task": OnyxCeleryTask.SYNC_LOCAL_MCP_TOOLS,
            "schedule": timedelta(seconds=LOCAL_MCP_TOOL_SYNC_INTERVAL_SECONDS),
            "options": {
                "priority": OnyxCeleryPriority.LOW,
                "expires": BEAT_EXPIRES_DEFAULT,
            },
