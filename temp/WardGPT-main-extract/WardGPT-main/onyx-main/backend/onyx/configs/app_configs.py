...
# Long-PDF multimodal context controls (chat-time selection).
PDF_MAX_EXTRACTED_IMAGES_PER_FILE = int(
    os.environ.get("PDF_MAX_EXTRACTED_IMAGES_PER_FILE", "300")
)
PDF_MAX_INLINE_PAGES = int(os.environ.get("PDF_MAX_INLINE_PAGES", "8"))
PDF_MAX_INLINE_IMAGES = int(os.environ.get("PDF_MAX_INLINE_IMAGES", "12"))
PDF_MAX_INLINE_TEXT_CHUNKS = int(os.environ.get("PDF_MAX_INLINE_TEXT_CHUNKS", "10"))
PDF_MAX_TEXT_CHARS_PER_CHUNK = int(
    os.environ.get("PDF_MAX_TEXT_CHARS_PER_CHUNK", "1200")
)
# Cap the image share of the remaining model input budget when selecting PDF visuals.
PDF_IMAGE_BUDGET_SHARE = float(os.environ.get("PDF_IMAGE_BUDGET_SHARE", "0.25"))
PDF_IMAGE_BUDGET_ABSOLUTE_MAX = int(
    os.environ.get("PDF_IMAGE_BUDGET_ABSOLUTE_MAX", "3500")
)
...
# Whether OAuth login is enabled (requires both client ID and secret)
OAUTH_ENABLED = bool(OAUTH_CLIENT_ID and OAUTH_CLIENT_SECRET)

_oauth_provider = (os.environ.get("OAUTH_PROVIDER") or "google").strip().lower()
if _oauth_provider not in {"google", "microsoft"}:
    logger.warning(
        "Invalid OAUTH_PROVIDER='%s'. Defaulting to 'google'.",
        _oauth_provider,
    )
    _oauth_provider = "google"
OAUTH_PROVIDER = _oauth_provider

# Tenant used by Microsoft Entra OAuth/OpenID routes.
# Example values: "common", "organizations", "<tenant-guid>"
OAUTH_MICROSOFT_TENANT_ID = (
    os.environ.get("OAUTH_MICROSOFT_TENANT_ID") or "common"
).strip()
...
# XLSX-specific indexing guardrails for SharePoint:
# index if row count is small enough OR file size is below this threshold.
SHAREPOINT_XLSX_MAX_ROWS_FOR_INDEXING = int(
    os.environ.get("SHAREPOINT_XLSX_MAX_ROWS_FOR_INDEXING", 1000)
)
SHAREPOINT_XLSX_MAX_FILE_SIZE_FOR_INDEXING = int(
    os.environ.get("SHAREPOINT_XLSX_MAX_FILE_SIZE_FOR_INDEXING", 5 * 1024 * 1024)
)
# Profile indexing controls for large XLSX files that exceed full-text limits.
SHAREPOINT_XLSX_PROFILE_MAX_SHEETS = int(
    os.environ.get("SHAREPOINT_XLSX_PROFILE_MAX_SHEETS", 25)
)
SHAREPOINT_XLSX_PROFILE_SAMPLE_ROWS_PER_SHEET = int(
    os.environ.get("SHAREPOINT_XLSX_PROFILE_SAMPLE_ROWS_PER_SHEET", 12)
)
SHAREPOINT_XLSX_PROFILE_MAX_COLUMNS = int(
    os.environ.get("SHAREPOINT_XLSX_PROFILE_MAX_COLUMNS", 16)
)
SHAREPOINT_XLSX_PROFILE_MAX_CELL_CHARS = int(
    os.environ.get("SHAREPOINT_XLSX_PROFILE_MAX_CELL_CHARS", 120)
)
SHAREPOINT_XLSX_PROFILE_MAX_ROW_SCAN_PER_SHEET = int(
    os.environ.get("SHAREPOINT_XLSX_PROFILE_MAX_ROW_SCAN_PER_SHEET", 10000)
)
...
AUTO_REGISTER_LOCAL_MCP_SERVER = (
    os.environ.get("AUTO_REGISTER_LOCAL_MCP_SERVER", "true").lower() == "true"
)
LOCAL_MCP_SERVER_ACTION_NAME = os.environ.get(
    "LOCAL_MCP_SERVER_ACTION_NAME", "wardGPT MCP by ProSource IT"
)
LOCAL_MCP_SERVER_URL = os.environ.get(
    "LOCAL_MCP_SERVER_URL", f"http://mcp_server:{MCP_SERVER_PORT}"
)
LOCAL_MCP_SERVER_OWNER_EMAIL = os.environ.get(
    "LOCAL_MCP_SERVER_OWNER_EMAIL", "system@wardgpt.local"
)
LOCAL_MCP_STATIC_API_KEY = os.environ.get("LOCAL_MCP_STATIC_API_KEY", "").strip()
LOCAL_MCP_API_KEY_ROTATION_ENABLED = (
    os.environ.get("LOCAL_MCP_API_KEY_ROTATION_ENABLED", "true").lower() == "true"
)
LOCAL_MCP_API_KEY_ROTATION_INTERVAL_SECONDS = int(
    os.environ.get("LOCAL_MCP_API_KEY_ROTATION_INTERVAL_SECONDS", "3600")
)
LOCAL_MCP_API_KEY_TTL_SECONDS = int(
    os.environ.get("LOCAL_MCP_API_KEY_TTL_SECONDS", "7200")
)
LOCAL_MCP_API_KEY_NAME_PREFIX = os.environ.get(
    "LOCAL_MCP_API_KEY_NAME_PREFIX", "wardgpt-local-mcp-service-key"
)
LOCAL_MCP_TOOL_SYNC_ENABLED = (
    os.environ.get("LOCAL_MCP_TOOL_SYNC_ENABLED", "true").lower() == "true"
)
LOCAL_MCP_TOOL_SYNC_INTERVAL_SECONDS = int(
    os.environ.get("LOCAL_MCP_TOOL_SYNC_INTERVAL_SECONDS", "600")
)
LOCAL_MCP_TOOL_SYNC_ON_STARTUP = (
    os.environ.get("LOCAL_MCP_TOOL_SYNC_ON_STARTUP", "true").lower() == "true"
)
LOCAL_MCP_TOOL_SYNC_STARTUP_DELAY_SECONDS = int(
    os.environ.get("LOCAL_MCP_TOOL_SYNC_STARTUP_DELAY_SECONDS", "20")
)
