...
class FileReaderTool(Tool[FileReaderToolOverrideKwargs]):
    ...
    def _validate_file_id(self, raw_file_id: str) -> UUID | str:
        if raw_file_id in self._chat_file_ids:
            return raw_file_id
    ...
    def _load_file(self, file_id: UUID | str) -> InMemoryChatFile:
        if file_id in self._user_file_ids:
            with get_session_with_current_tenant() as db_session:
                return load_user_file(cast(UUID, file_id), db_session)
