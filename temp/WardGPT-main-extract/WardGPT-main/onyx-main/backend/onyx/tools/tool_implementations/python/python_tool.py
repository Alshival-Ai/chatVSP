...
def _is_kmz_archive(content: bytes) -> bool:
    """Best-effort KMZ detection for extensionless generated files."""
    try:
        with zipfile.ZipFile(BytesIO(content)) as archive:
            return any(
                (not info.is_dir()) and info.filename.lower().endswith(".kml")
...
def _normalize_generated_filename(filename: str, content: bytes) -> str:
    """Ensure generated KMZ outputs have a .kmz extension."""
    normalized = (filename or "").strip() or "output"
    if Path(normalized).suffix:
        return normalized
    if _is_kmz_archive(content):
        return f"{normalized}.kmz"
    return normalized
            ...
            try:
                ...
                for workspace_file in result_event.files:
                    ...
                    try:
                        ...
                        # Determine normalized filename + MIME type.
                        raw_filename = workspace_file.path.split("/")[-1]
                        filename = _normalize_generated_filename(
                            filename=raw_filename,
                            content=file_content,
                        )
                        mime_type, _ = mimetypes.guess_type(filename)
                        # Default to KMZ for normalized KMZ files, otherwise binary.
                        mime_type = (
                            mime_type
                            or (KMZ_MIME_TYPE if filename.lower().endswith(".kmz") else None)
                    ...
                    self.emitter.emit(
                        Packet(
                            ...
                            obj=PythonToolDelta(
                                ...
                                file_names=[f.filename for f in generated_files],
                            ),
                        )
                    )
