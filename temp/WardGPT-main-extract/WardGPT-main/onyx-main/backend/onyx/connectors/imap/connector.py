...
_AUTH_METHOD_KEY = "authentication_method"
_AUTH_METHOD_OAUTH_INTERACTIVE = "oauth_interactive"
_GRAPH_API_BASE = "https://graph.microsoft.com/v1.0"
_GRAPH_DEFAULT_INITIAL_DAYS = 90
_GRAPH_OAUTH_SCOPES = "offline_access Mail.Read"
_GRAPH_DEFAULT_SELECT = (
    "id,subject,from,toRecipients,ccRecipients,bccRecipients,body,webLink,"
    "lastModifiedDateTime,receivedDateTime,internetMessageId"
)
...
class ImapCheckpoint(ConnectorCheckpoint):
    ...
    graph_next_link: str | None = None
    graph_default_window_applied: bool = False
...
class ImapConnector(
    ...
    def oauth_authorization_url(
        ...
        if OAUTH_PROVIDER != "microsoft":
            raise ValueError("Outlook OAuth requires OAUTH_PROVIDER=microsoft")
        if not OAUTH_CLIENT_ID:
            raise ValueError("OAUTH_CLIENT_ID must be configured")

        callback_uri = get_oauth_callback_uri(base_domain, "imap")
    ...
    def oauth_code_to_token(
        ...
        response = requests.post(
            ...
            data={
                ...
                "redirect_uri": callback_uri,
                "scope": _GRAPH_OAUTH_SCOPES,
            },
            timeout=REQUEST_TIMEOUT_SECONDS,
        )
        if not response.ok:
            raise RuntimeError(f"Failed to exchange OAuth code: {response.text}")

        token_data = response.json()
        expires_in = int(token_data.get("expires_in", 3600))
        return {
            _AUTH_METHOD_KEY: _AUTH_METHOD_OAUTH_INTERACTIVE,
    ...
    def _is_oauth_mode(self) -> bool:
        return (
            self.credentials.get(_AUTH_METHOD_KEY) == _AUTH_METHOD_OAUTH_INTERACTIVE
            and bool(self.credentials.get("access_token"))
        )
    ...
    def _persist_credentials(self) -> None:
        if self._credentials_provider is not None and self._credentials is not None:
            self._credentials_provider.set_credentials(self._credentials)

    def _get_valid_graph_access_token(self) -> str:
        access_token = self.credentials.get("access_token")
        if not access_token or not isinstance(access_token, str):
            raise RuntimeError("OAuth access_token is missing")

        expires_at_raw = self.credentials.get("expires_at")
        ...
        if not refresh_token or not isinstance(refresh_token, str):
            raise RuntimeError("OAuth refresh_token is missing")
        if OAUTH_PROVIDER != "microsoft":
            raise RuntimeError("Outlook OAuth requires OAUTH_PROVIDER=microsoft")
        if not OAUTH_CLIENT_ID or not OAUTH_CLIENT_SECRET:
            raise RuntimeError(
                "OAUTH_CLIENT_ID and OAUTH_CLIENT_SECRET are required to refresh OAuth token"
            )
        ...
        if not refresh_resp.ok:
            raise RuntimeError(f"Failed to refresh OAuth token: {refresh_resp.text}")
        ...
        self.credentials["expires_at"] = int(time.time()) + max(0, expires_in - 60)
    ...
    def _graph_get_json(
        ...
        access_token = self._get_valid_graph_access_token()
        ...
        if not response.ok:
            raise RuntimeError(
                f"Graph mail request failed ({response.status_code}): {response.text}"
            )
        return cast(dict[str, Any], response.json())

    def _resolve_graph_mail_folder(self, mailbox: str) -> str:
        mailbox_l = mailbox.strip().lower().strip('"')
        known = {
            "inbox": "inbox",
            "sent": "sentitems",
            "sentitems": "sentitems",
        ...
        if mailbox_l in known:
            return known[mailbox_l]

        folder_data = self._graph_get_json(
            f"{_GRAPH_API_BASE}/me/mailFolders",
            params={
                "$select": "id,displayName",
                "$top": "200",
            },
        )
        for folder in folder_data.get("value", []):
            display_name = str(folder.get("displayName", "")).strip().lower()
            if display_name == mailbox_l:
                return cast(str, folder["id"])
        raise RuntimeError(f"Could not resolve mailbox '{mailbox}' in Graph folders")

    def _effective_window_for_graph(
        self, start: SecondsSinceUnixEpoch, checkpoint: ImapCheckpoint
    ) -> SecondsSinceUnixEpoch:
        if checkpoint.graph_default_window_applied:
            return start
        if start > 0:
            return start
        checkpoint.graph_default_window_applied = True
        ninety_days_ago = datetime.now(tz=timezone.utc) - timedelta(
            days=_GRAPH_DEFAULT_INITIAL_DAYS
        )
        return ninety_days_ago.timestamp()

    def _convert_graph_message_to_document(
        self, message: dict[str, Any], include_perm_sync: bool
    ) -> Document:
        from_field = (
        ...
        sender_name = str(from_field.get("name", "") or "")
        sender_addr = str(from_field.get("address", "") or "")

        recipients: list[tuple[str, str]] = []
        for recipient_field in ["toRecipients", "ccRecipients", "bccRecipients"]:
            value = message.get(recipient_field, [])
            ...
            for recipient in value:
                ...
                addr = str(email_address.get("address", "") or "")
                if not addr:
                    continue
                recipients.append((str(email_address.get("name", "") or ""), addr))

        expert_info_map: dict[str, BasicExpertInfo] = {
            addr: BasicExpertInfo(display_name=name, email=addr)
        ...
        body_text = (
            " ".join(
                str_section
                for str_section in bs4.BeautifulSoup(
                    markup=body_content, features="html.parser"
                ).stripped_strings
            )
            if body_type == "html"
            else body_content
        ...
        if modified_raw:
            doc_updated_at = datetime.fromisoformat(modified_raw.replace("Z", "+00:00"))
        ...
        if not message_id:
            raise RuntimeError("Graph message is missing id")
        ...
        return Document(
            ...
            metadata={"web_link": web_link} if web_link else {},
            source=DocumentSource.IMAP,
            sections=[TextSection(text=body_text, link=web_link or None)],
            primary_owners=list(expert_info_map.values()),
            external_access=external_access,
            doc_updated_at=doc_updated_at,
        )

    def _load_from_checkpoint_graph(
        self,
        ...
        if checkpoint.todo_mailboxes is None:
            checkpoint.todo_mailboxes = (
                [mailbox.strip() for mailbox in self._mailboxes if mailbox.strip()]
                if self._mailboxes
                else ["INBOX"]
            )

        if not checkpoint.current_mailbox or not checkpoint.current_mailbox.mailbox:
            if not checkpoint.todo_mailboxes:
                checkpoint.has_more = False
                return checkpoint
            next_mailbox = checkpoint.todo_mailboxes.pop(0)
            checkpoint.current_mailbox = CurrentMailbox(
                mailbox=next_mailbox, todo_email_ids=[]
            )
            checkpoint.graph_next_link = None

        if checkpoint.current_mailbox is None:
            checkpoint.has_more = False
            return checkpoint

        if checkpoint.graph_next_link:
            url = checkpoint.graph_next_link
            params = None
        else:
            mailbox_path = self._resolve_graph_mail_folder(checkpoint.current_mailbox.mailbox)
            effective_start = self._effective_window_for_graph(start, checkpoint)
            start_iso = datetime.fromtimestamp(
                effective_start, tz=timezone.utc
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
            end_iso = datetime.fromtimestamp(end, tz=timezone.utc).strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            )
            url = f"{_GRAPH_API_BASE}/me/mailFolders/{mailbox_path}/messages"
            params = {
                "$select": _GRAPH_DEFAULT_SELECT,
                "$orderby": "lastModifiedDateTime asc",
                "$top": str(_PAGE_SIZE),
                "$filter": (
                    f"lastModifiedDateTime ge {start_iso} "
                    f"and lastModifiedDateTime lt {end_iso}"
                ),
            }

        data = self._graph_get_json(url=url, params=params)
        for message in data.get("value", []):
            if not isinstance(message, dict):
                continue
            yield self._convert_graph_message_to_document(
                message=message, include_perm_sync=include_perm_sync
            )

        checkpoint.graph_next_link = cast(str | None, data.get("@odata.nextLink"))
        if checkpoint.graph_next_link is None:
            checkpoint.current_mailbox = None

        checkpoint.has_more = bool(
            checkpoint.graph_next_link
            or checkpoint.current_mailbox is not None
            or checkpoint.todo_mailboxes
        )
        return checkpoint
    ...
    def _load_from_checkpoint(
        ...
        if self._is_oauth_mode():
            return self._load_from_checkpoint_graph(
                start=start,
                end=end,
    ...
    def validate_connector_settings(self) -> None:
        if self._is_oauth_mode():
            self._graph_get_json(
                f"{_GRAPH_API_BASE}/me/mailFolders/inbox",
                params={"$select": "id"},
            )
            return
