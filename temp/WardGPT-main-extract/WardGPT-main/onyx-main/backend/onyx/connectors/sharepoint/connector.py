...
from onyx.configs.app_configs import OAUTH_MICROSOFT_TENANT_ID
from onyx.configs.app_configs import OAUTH_PROVIDER
from onyx.configs.app_configs import REQUEST_TIMEOUT_SECONDS
from onyx.configs.app_configs import SHAREPOINT_CONNECTOR_SIZE_THRESHOLD
from onyx.configs.app_configs import SHAREPOINT_XLSX_MAX_FILE_SIZE_FOR_INDEXING
from onyx.configs.app_configs import SHAREPOINT_XLSX_MAX_ROWS_FOR_INDEXING
from onyx.configs.app_configs import SHAREPOINT_XLSX_PROFILE_MAX_CELL_CHARS
from onyx.configs.app_configs import SHAREPOINT_XLSX_PROFILE_MAX_COLUMNS
from onyx.configs.app_configs import SHAREPOINT_XLSX_PROFILE_MAX_ROW_SCAN_PER_SHEET
from onyx.configs.app_configs import SHAREPOINT_XLSX_PROFILE_MAX_SHEETS
from onyx.configs.app_configs import SHAREPOINT_XLSX_PROFILE_SAMPLE_ROWS_PER_SHEET
...
class SharepointAuthMethod(Enum):
    ...
    MICROSOFT_GRAPH_BACKEND = "microsoft_graph_backend"
...
def _get_microsoft_graph_backend_credentials() -> tuple[str, str, str]:
    if OAUTH_PROVIDER != "microsoft":
        raise ConnectorValidationError(
            "Microsoft Graph backend credentials are only available when "
            "OAUTH_PROVIDER is set to 'microsoft'."
        )
    if not OAUTH_CLIENT_ID or not OAUTH_CLIENT_SECRET:
        raise ConnectorValidationError(
            "Microsoft Graph backend credentials are not configured. "
            "Set OAUTH_CLIENT_ID and OAUTH_CLIENT_SECRET."
        )

    tenant_id = OAUTH_MICROSOFT_TENANT_ID.strip()
    if tenant_id in {"", "common", "organizations", "consumers"}:
        raise ConnectorValidationError(
            "OAUTH_MICROSOFT_TENANT_ID must be a specific tenant ID "
            "when using Microsoft Graph backend credentials."
        )

    return OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET, tenant_id
...
def _count_non_empty_xlsx_rows(content_bytes: bytes, max_rows: int) -> int | None:
    """Count non-empty rows in an XLSX payload up to max_rows + 1.

    Returns:
    - row count (possibly max_rows + 1 when limit exceeded)
    - None when workbook cannot be parsed
    """
    try:
        ...
        logger.warning("Failed to inspect XLSX row count: %s", e)
        return None
...
def _stringify_xlsx_cell(value: Any, max_chars: int) -> str:
    cell_text = str(value).strip() if value is not None else ""
    if len(cell_text) > max_chars:
        return cell_text[: max_chars - 3] + "..."
    return cell_text


def _build_xlsx_profile_sections(
    content_bytes: bytes, file_name: str
) -> list[str] | None:
    """Build bounded workbook/sheet profile sections for XLSX indexing.

    For large workbooks we can index these sections directly. For normal
    workbooks we can append sheet profiles to improve sheet-level retrieval.
    """
    try:
        workbook = openpyxl.load_workbook(
            io.BytesIO(content_bytes), read_only=True, data_only=True
        ...
        logger.warning("Failed to build XLSX profile sections for %s: %s", file_name, e)
        return None

    overview_lines = [
        f"Workbook: {file_name}",
        "Workbook profile: bounded sheet metadata and sampled non-empty rows.",
        "",
    ]
    ...
    try:
        ...
        overview_lines.append(f"Total sheets: {total_sheet_count}")
        if sheets:
            indexed_sheet_names = [
                sheet.title for sheet in sheets[:SHAREPOINT_XLSX_PROFILE_MAX_SHEETS]
            ]
            overview_lines.append(
                "Indexed sheet names: " + ", ".join(indexed_sheet_names)
            )
            if total_sheet_count > SHAREPOINT_XLSX_PROFILE_MAX_SHEETS:
                overview_lines.append(
                    f"Additional sheets omitted: {total_sheet_count - SHAREPOINT_XLSX_PROFILE_MAX_SHEETS}"
                )

        section_texts.append("\n".join(overview_lines).strip())

        for sheet_index, sheet in enumerate(sheets):
            if sheet_index >= SHAREPOINT_XLSX_PROFILE_MAX_SHEETS:
                break
            ...
            for row_idx, row in enumerate(
                ...
                scanned_rows += 1
                if scanned_rows > SHAREPOINT_XLSX_PROFILE_MAX_ROW_SCAN_PER_SHEET:
                    scan_truncated = True
                    break

                rendered_row = [
                    _stringify_xlsx_cell(value, SHAREPOINT_XLSX_PROFILE_MAX_CELL_CHARS)
                    for value in row[:SHAREPOINT_XLSX_PROFILE_MAX_COLUMNS]
                ]
                if not any(rendered_row):
                    continue

                non_empty_rows += 1
                max_columns_seen = max(max_columns_seen, len(rendered_row))

                if header_values is None:
                    header_values = rendered_row

                if len(sampled_rows) < SHAREPOINT_XLSX_PROFILE_SAMPLE_ROWS_PER_SHEET:
                    sampled_rows.append((row_idx, rendered_row))
            ...
            sheet_lines.append(
                f"Non-empty rows scanned: {non_empty_rows}"
                + (" (scan capped)" if scan_truncated else "")
            )
            sheet_lines.append(f"Max columns observed: {max_columns_seen}")
            if header_values:
                header_preview = " | ".join(v for v in header_values if v)
                if header_preview:
                    sheet_lines.append(f"Header preview: {header_preview}")

            if sampled_rows:
                sheet_lines.append("Sample rows:")
                for row_idx, row_values in sampled_rows:
                    rendered = " | ".join(v for v in row_values if v)
                    if rendered:
                        sheet_lines.append(f"- Row {row_idx}: {rendered}")
            else:
                sheet_lines.append("Sample rows: (none)")
...
def _build_xlsx_profile_text(content_bytes: bytes, file_name: str) -> str | None:
    """Backward-compatible single-string XLSX profile helper."""
    sections = _build_xlsx_profile_sections(content_bytes, file_name)
    if not sections:
        return None
    return "\n\n".join(sections)
...
def _convert_driveitem_to_document_with_permissions(
    ...
    if not content_bytes:
        ...
        # Keep SharePoint XLSX indexing bounded: index only if row-count is modest
        # or the file is small enough.
        use_xlsx_profile_only = False
        if file_ext == ".xlsx":
            effective_size = file_size if file_size is not None else len(content_bytes)
            if effective_size > SHAREPOINT_XLSX_MAX_FILE_SIZE_FOR_INDEXING:
                row_count = _count_non_empty_xlsx_rows(
                    content_bytes, SHAREPOINT_XLSX_MAX_ROWS_FOR_INDEXING
                )
                if (
                    row_count is None
                    or row_count > SHAREPOINT_XLSX_MAX_ROWS_FOR_INDEXING
                ):
                    logger.warning(
                        "Using XLSX profile fallback for '%s': size=%s bytes, rows=%s (max_rows=%s, max_small_file_size=%s).",
                        driveitem.name,
                        effective_size,
                        "unknown" if row_count is None else row_count,
                        SHAREPOINT_XLSX_MAX_ROWS_FOR_INDEXING,
                        SHAREPOINT_XLSX_MAX_FILE_SIZE_FOR_INDEXING,
                    )
                    use_xlsx_profile_only = True
        ...
        if use_xlsx_profile_only:
            profile_sections = _build_xlsx_profile_sections(
                content_bytes, driveitem.name
            )
            if profile_sections:
                for profile_text in profile_sections:
                    sections.append(
                        TextSection(link=driveitem.web_url, text=profile_text)
                    )
            else:
                logger.warning(
                    "Failed to build XLSX profile for '%s'; skipping document.",
                    driveitem.name,
                )
                return None
        else:
            ...
            if extraction_result.text_content:
                ...
                # Make filename-based lookup reliable for spreadsheets and attach
                # bounded sheet-level profiles for better multi-sheet retrieval.
                if file_ext == ".xlsx":
                    profile_sections = _build_xlsx_profile_sections(
                        content_bytes, driveitem.name
                    )
                    if profile_sections:
                        text_content = f"{profile_sections[0]}\n\n{text_content}"
                        for sheet_profile_text in profile_sections[1:]:
                            sections.append(
                                TextSection(
                                    link=driveitem.web_url, text=sheet_profile_text
                                )
                            )
                    else:
                        text_content = f"Workbook: {driveitem.name}\n\n{text_content}"
                sections.append(
                    TextSection(
                        link=driveitem.web_url, text=text_content
                    )
                )

    if not sections:
        logger.warning("No extractable content for '%s'; skipping document.", driveitem.name)
        return None
...
class SharepointConnector(
    ...
    def __init__(
        ...
        self._oauth_expires_at: float | None = None
    ...
    def oauth_authorization_url(
        ...
        if OAUTH_PROVIDER != "microsoft":
            raise ValueError(
                "SharePoint delegated OAuth requires OAUTH_PROVIDER=microsoft"
            )
        if not OAUTH_CLIENT_ID:
            raise ValueError("OAUTH_CLIENT_ID must be configured")

        redirect_uri = get_oauth_callback_uri(base_domain, "sharepoint")
        tenant_id = OAUTH_MICROSOFT_TENANT_ID.strip() or "organizations"
        scopes = [
            "openid",
            "email",
            "profile",
        ...
        return f"{DEFAULT_AUTHORITY_HOST}/{tenant_id}/oauth2/v2.0/authorize?{query}"
    ...
    def oauth_code_to_token(
        ...
        redirect_uri = get_oauth_callback_uri(base_domain, "sharepoint")
        tenant_id = OAUTH_MICROSOFT_TENANT_ID.strip() or "organizations"
        token_url = f"{DEFAULT_AUTHORITY_HOST}/{tenant_id}/oauth2/v2.0/token"
        ...
        if not resp.ok:
            raise RuntimeError(f"Failed to exchange OAuth code: {resp.text}")

        token_data = resp.json()
        expires_in = int(token_data.get("expires_in", 3600))
        return {
            "authentication_method": SharepointAuthMethod.OAUTH_INTERACTIVE.value,
    ...
    def _acquire_token(self) -> dict[str, Any]:
        ...
        """
        if self._oauth_access_token is not None:
            return {"access_token": self._get_valid_oauth_access_token()}
    ...
    def _get_valid_oauth_access_token(self) -> str:
        if not self._oauth_access_token:
            raise ConnectorValidationError("SharePoint OAuth access token is missing")

        now = time.time()
        if self._oauth_expires_at and now < self._oauth_expires_at:
            return self._oauth_access_token

        if not self._oauth_refresh_token:
            raise ConnectorValidationError(
                "SharePoint OAuth access token expired and no refresh token is available"
            )
        if OAUTH_PROVIDER != "microsoft":
            raise ConnectorValidationError(
                "SharePoint delegated OAuth requires OAUTH_PROVIDER=microsoft"
        ...
        if not OAUTH_CLIENT_ID or not OAUTH_CLIENT_SECRET:
            raise ConnectorValidationError(
                "OAUTH_CLIENT_ID and OAUTH_CLIENT_SECRET are required to refresh SharePoint OAuth tokens"
            )

        tenant_id = OAUTH_MICROSOFT_TENANT_ID.strip() or "organizations"
        token_url = f"{self.authority_host}/{tenant_id}/oauth2/v2.0/token"
        ...
        if not refresh_resp.ok:
            raise ConnectorValidationError(
                f"Failed to refresh SharePoint OAuth token: {refresh_resp.text}"
            )

        refreshed = refresh_resp.json()
        self._oauth_access_token = refreshed["access_token"]
        self._oauth_refresh_token = refreshed.get(
            "refresh_token", self._oauth_refresh_token
        )
    ...
    def load_credentials(self, credentials: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if auth_method == SharepointAuthMethod.OAUTH_INTERACTIVE.value:
            self._oauth_access_token = credentials.get("access_token")
            self._oauth_refresh_token = credentials.get("refresh_token")
            raw_expires_at = credentials.get("expires_at")
            self._oauth_expires_at = (
                float(raw_expires_at) if raw_expires_at is not None else None
            )
            if not self._oauth_access_token:
                raise ConnectorValidationError(
                    "SharePoint OAuth interactive credential is missing access_token"
                )

            def _acquire_oauth_token_for_graph() -> dict[str, Any]:
                return {"access_token": self._get_valid_oauth_access_token()}

            self._graph_client = GraphClient(
                _acquire_oauth_token_for_graph, environment=self._azure_environment
            )
            return {
                **credentials,
                "access_token": self._get_valid_oauth_access_token(),
                "refresh_token": self._oauth_refresh_token,
                "expires_at": int(self._oauth_expires_at or time.time() + 3000),
            }
        ...
        if auth_method == SharepointAuthMethod.MICROSOFT_GRAPH_BACKEND.value:
            (
                sp_client_id,
                sp_client_secret,
                sp_directory_id,
            ) = _get_microsoft_graph_backend_credentials()
            logger.info("Using Microsoft Graph backend credentials")
        elif (
            auth_method == SharepointAuthMethod.CLIENT_SECRET.value
            and not (sp_client_id and sp_client_secret and sp_directory_id)
        ):
            # Backward-compatible fallback for credentials created before the
            # dedicated backend auth mode existed.
            (
                sp_client_id,
                sp_client_secret,
                sp_directory_id,
            ...
            logger.info(
                "SharePoint client-secret fields are incomplete; "
                "falling back to Microsoft Graph backend credentials"
            )
        ...
        if auth_method == SharepointAuthMethod.CERTIFICATE.value:
            ...
            if not sp_client_secret:
                raise ConnectorValidationError("Client Secret is required")
            self.msal_app = msal.ConfidentialClientApplication(
                authority=authority_url,
                client_id=sp_client_id,
                client_credential=sp_client_secret,
            )
        elif auth_method == SharepointAuthMethod.MICROSOFT_GRAPH_BACKEND.value:
            if not sp_client_secret:
                raise ConnectorValidationError(
                    "Microsoft Graph backend client secret is missing"
                )
            self.msal_app = msal.ConfidentialClientApplication(
                authority=authority_url,
                client_id=sp_client_id,
    ...
    def _load_from_checkpoint(
        ...
        if (
            ...
            for driveitem in driveitems:
                ...
                # Delta responses can omit parentReference.driveId. We still know
                # which drive we are traversing, so backfill it for Graph /content
                # fallback downloads.
                if not driveitem.drive_id:
                    driveitem.drive_id = checkpoint.current_drive_id
