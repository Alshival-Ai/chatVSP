...
from onyx.llm.models import FileContentPart
...
_OPENAI_MAX_FILE_BYTES = 50 * 1024 * 1024
_KMZ_WORKBOOK_TEMPLATE_RUNTIME_FILE_ID = "__kmz_workbook_template__"
_OPENAI_ALLOWED_FILE_EXTENSIONS = {
    ".pdf",
    ".txt",
    ".md",
    ".json",
            ...
            if isinstance(msg.content, str):
                ...
                for part in msg.content:
                    if isinstance(part, TextContentPart):
                        ...
                        formatted_lines.append(f"[File: {part.filename}]")
...
def _is_openai_file_input_candidate(filename: str | None) -> bool:
    if not filename:
        return False
    extension = os.path.splitext(filename)[1].lower()
    return extension in _OPENAI_ALLOWED_FILE_EXTENSIONS
...
def translate_history_to_llm_format(
    ...
    is_openai_provider = (
        llm_config.model_provider == LlmProviderNames.AZURE
        or is_true_openai_model(llm_config.model_provider, llm_config.model_name)
    )
    total_openai_file_bytes = 0
    ...
    for idx, msg in enumerate(history):
        ...
        if msg.message_type == MessageType.SYSTEM:
            ...
            # Handle user messages with potential images/files
            has_images = bool(msg.image_files)
            has_non_image_files = bool(msg.non_image_files)
            if has_images or has_non_image_files:
                # Build content parts: text + images + files.
                content_parts: list[TextContentPart | ImageContentPart | FileContentPart] = [  # noqa: E501
                    TextContentPart(
                        type="text",
                        text=msg.message,
                    )
                ...
                # Add OpenAI input_file parts for eligible non-image attachments.
                if is_openai_provider and msg.non_image_files:
                    for attached_file in msg.non_image_files:
                        # Legacy KMZ template workbook is a synthetic runtime-only file.
                        # It is not suitable for OpenAI `input_file` chat payloads.
                        if attached_file.file_id == _KMZ_WORKBOOK_TEMPLATE_RUNTIME_FILE_ID:
                            continue
                        filename = attached_file.filename
                        if not _is_openai_file_input_candidate(filename):
                            continue

                        file_size = len(attached_file.content)
                        if file_size > _OPENAI_MAX_FILE_BYTES:
                            logger.warning(
                                "Skipping oversized file '%s' (%s bytes) for OpenAI input_file.",
                                filename,
                                file_size,
                            )
                            continue
                        if total_openai_file_bytes + file_size > _OPENAI_MAX_FILE_BYTES:
                            logger.warning(
                                "Skipping file '%s' to keep total OpenAI input_file payload under 50MB.",
                                filename,
                            )
                            continue
                            ...
                            content_parts.append(
                                FileContentPart(
                                    ...
                                    file_data=_build_openai_file_data_uri(
                                        filename,
                                        attached_file.to_base64(),
                                    ),
                                    filename=filename,
                                )
                            )
                            total_openai_file_bytes += file_size
                        except Exception as e:
                            logger.warning(
                                f"Failed to encode file {attached_file.file_id} as input_file: {e}. "
                                "Skipping file."
                            )

                user_msg = UserMessage(role="user", content=content_parts)
