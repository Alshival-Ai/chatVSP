...
def convert_chat_history(
    ...
    disable_openai_input_file_parts: bool = False,
) -> ChatHistoryResult:
    ...
    is_openai_provider = (
        not disable_openai_input_file_parts
        and llm_model_provider
        in {
    ...
    for idx, chat_message in enumerate(chat_history):
        if chat_message.message_type == MessageType.USER:
            ...
            input_file_non_image_files: list[ChatLoadedFile] = []
                ...
                for file_descriptor in chat_message.files:
                    ...
                    if loaded_file:
                        if loaded_file.file_type == ChatFileType.IMAGE:
                            ...
                            # OpenAI input_file candidates should be included only as file parts,
                            # not duplicated as injected "File: ... End of File" text context.
                            is_openai_input_file = (
                                is_openai_provider
                                and _is_openai_file_input_candidate(
                                    loaded_file.filename
                                )
                            )
                            if is_openai_input_file:
                                input_file_non_image_files.append(loaded_file)
                            else:
            ...
            # Add the user message with image files and non-image attachments.
            ...
            # Sum token counts from all attached image files, including context images.
            image_token_count = (
            ...
            simple_messages.append(
                ChatMessageSimple(
                    ...
                    non_image_files=(
                        input_file_non_image_files
                        if input_file_non_image_files
                        else None
                    ),
                )
            )
