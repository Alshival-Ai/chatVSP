...
_PLACEHOLDER_MARKDOWN_LINK_PATTERN = re.compile(
    r"\[([^\]]+)\]\((file_link/?|https?://file_link/?)\)",
    flags=re.IGNORECASE,
)
...
) -> Iterable[PythonExecutionFile]:
    for tool_call_info in tool_calls:
        if not tool_call_info.generated_files:
            continue
        for generated_file in tool_call_info.generated_files:
            yield generated_file


def _rewrite_generated_file_paths_to_links(
    message_text: str,
    tool_calls: list[ToolCallInfo],
) -> str:
    """Rewrite local/sandbox generated file paths to downloadable markdown links."""
    if not message_text:
        return message_text

    rewritten_text = message_text
    generated_files = [
        generated_file
        for generated_file in _iter_generated_files(tool_calls)
        if generated_file.file_link and generated_file.filename
    ]

    for generated_file in generated_files:
        # Some responses emit placeholders like:
        # [compiled_packets.kmz](file_link) or [compiled_packets.kmz](https://file_link/)
        for placeholder_url in _PLACEHOLDER_FILE_LINK_URLS:
            rewritten_text = rewritten_text.replace(
                f"[{generated_file.filename}]({placeholder_url})",
                f"[{generated_file.filename}]({generated_file.file_link})",
            )

        # Models often output local runtime paths (sandbox:/mnt/data or /mnt/data).
        # Rewrite these to stable chat file URLs so frontend download links work.
        candidate_paths = (
            f"sandbox:/mnt/data/{generated_file.filename}",
            f"/mnt/data/{generated_file.filename}",
        )
        markdown_link = f"[{generated_file.filename}]({generated_file.file_link})"

        for candidate_path in candidate_paths:
            rewritten_text = rewritten_text.replace(
                f"({candidate_path})", f"({generated_file.file_link})"
            )
            rewritten_text = rewritten_text.replace(candidate_path, markdown_link)

    # Fallback: map any remaining placeholder links by generated-file order.
    if generated_files:
        unresolved_generated_files = [
            generated_file
            for generated_file in generated_files
            if generated_file.file_link not in rewritten_text
        ]
        available_generated_files = list(unresolved_generated_files or generated_files)
        used_file_links: set[str] = set()

        def _desired_extension_for_placeholder(
            *,
            label: str,
            context_prefix: str,
        ) -> str | None:
            lower_label = label.lower()
            lower_context = context_prefix.lower()
            combined = f"{lower_context} {lower_label}"
        ...
        def _select_generated_file_for_placeholder(
            *,
            desired_extension: str | None,
        ) -> PythonExecutionFile | None:
            # If the message context implies a specific file type, only use a file
            # with that extension. This prevents mislabeling, e.g. KMZ placeholders
            # accidentally resolving to XLSX links.
            if desired_extension:
                for generated_file in available_generated_files:
                    if generated_file.file_link in used_file_links:
                        continue
                    if generated_file.filename.lower().endswith(desired_extension):
                        used_file_links.add(generated_file.file_link)
        ...
        def _replace_placeholder_markdown_link(match: re.Match[str]) -> str:
            label = (match.group(1) or "").strip()
            context_start = max(0, match.start() - 120)
            context_prefix = rewritten_text[context_start : match.start()]
            desired_extension = _desired_extension_for_placeholder(
                label=label,
                context_prefix=context_prefix,
            )

            generated_file = _select_generated_file_for_placeholder(
                desired_extension=desired_extension,
            )
            if generated_file is None:
                return match.group(0)

            if label.lower() in {"file_link", "download", "download file", "download link"}:
                label = generated_file.filename
            return f"[{label}]({generated_file.file_link})"

        rewritten_text = _PLACEHOLDER_MARKDOWN_LINK_PATTERN.sub(
            _replace_placeholder_markdown_link, rewritten_text
        )
...
def _extract_referenced_file_descriptors(
    ...
    for tool_call_info in tool_calls:
        ...
        for gen_file in tool_call_info.generated_files:
            ...
            if file_id and file_id in message_text and file_id not in seen_ids:
...
def save_chat_turn(
    ...
    normalized_message_text = _rewrite_generated_file_paths_to_links(
        message_text=message_text,
        tool_calls=tool_calls,
    )
    sanitized_message_text = (
        sanitize_string(normalized_message_text)
        if normalized_message_text
        else normalized_message_text
    )
