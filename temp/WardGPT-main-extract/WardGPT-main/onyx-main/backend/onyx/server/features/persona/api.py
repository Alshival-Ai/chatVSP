...
from onyx.db.critical_agents import CriticalAgentKey
from onyx.db.critical_agents import DeployStatus
from onyx.db.critical_agents import deploy_critical_agent
...
@admin_agents_router.post("/deploy", tags=PUBLIC_API_TAGS)
def deploy_agent(
    deploy_agent_request: DeployAgentRequest,
    _: User = Depends(current_admin_user),
    db_session: Session = Depends(get_session),
) -> DeployAgentResponse:
    agent_name, status = deploy_critical_agent(
        db_session=db_session, agent=deploy_agent_request.agent
    )
    return DeployAgentResponse(agent_name=agent_name, status=status)
