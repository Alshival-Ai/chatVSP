...
def is_onyx_craft_enabled(user: User) -> bool:
    ...
    """
    # Global hard gate: when runtime Craft support is disabled, no users should
    # see or access Craft regardless of per-user or PostHog flags.
    if not ENABLE_CRAFT:
        return False

    # Per-user access gate: admin controls Craft visibility from User Management.
    if not user.enable_onyx_craft:
        return False
