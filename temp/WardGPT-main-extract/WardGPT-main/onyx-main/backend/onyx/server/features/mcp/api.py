...
def _looks_masked_secret(value: str | None) -> bool:
    """Heuristic check for UI-masked secrets (e.g. abcd...wxyz, ••••)."""
    if not value:
        return False

    stripped = value.strip()
    return "..." in stripped or stripped == "••••••••••••"
...
def _upsert_mcp_server(
    ...
    existing_admin_api_token: str | None = None
    ...
    if request.existing_server_id:
        ...
        if mcp_server.admin_connection_config:
            ...
            existing_auth_header = existing_admin_config_dict.get("headers", {}).get(
                "Authorization"
            )
            if (
                mcp_server.auth_type == MCPAuthenticationType.API_TOKEN
                and existing_auth_header
                and existing_auth_header.startswith("Bearer ")
            ):
                existing_admin_api_token = existing_auth_header.split(" ", 1)[1]
        ...
        requested_admin_api_token = request.api_token
        if (
            request.auth_type == MCPAuthenticationType.API_TOKEN
            and request.auth_performer == MCPAuthenticationPerformer.ADMIN
            and (
                not requested_admin_api_token
                or _looks_masked_secret(requested_admin_api_token)
            )
        ):
            # Preserve existing token when UI sends masked/blank values on edit.
            requested_admin_api_token = existing_admin_api_token
        ...
        changing_connection_config = (
            ...
            or (
                request.auth_type == MCPAuthenticationType.API_TOKEN
                and request.auth_performer == MCPAuthenticationPerformer.ADMIN
                and requested_admin_api_token != existing_admin_api_token
            )
        ...
        # Carry forward for connection-config creation below.
        if (
            request.auth_type == MCPAuthenticationType.API_TOKEN
            and request.auth_performer == MCPAuthenticationPerformer.ADMIN
        ):
            request.api_token = requested_admin_api_token
