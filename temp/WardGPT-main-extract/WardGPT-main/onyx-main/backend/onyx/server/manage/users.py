        ...
        return AllUsersResponse(
            accepted=[
                FullUserSnapshot(
                    ...
                    enable_onyx_craft=user.enable_onyx_craft,
...
def update_user_code_interpreter_access(
    ...
    if user_to_update.enable_code_interpreter == request.enabled:
        return

    user_to_update.enable_code_interpreter = request.enabled
    db_session.commit()


@router.patch("/manage/admin/onyx-craft-access", tags=PUBLIC_API_TAGS)
def update_user_onyx_craft_access(
    ...
    if request.enabled and not ENABLE_CRAFT:
        raise HTTPException(
            status_code=400,
            detail="Onyx Craft is globally disabled",
        )

    # When Craft is globally disabled, always persist False to avoid stale grants.
    target_enabled = request.enabled if ENABLE_CRAFT else False

    if user_to_update.enable_onyx_craft == target_enabled:
        return

    user_to_update.enable_onyx_craft = target_enabled
    db_session.commit()
...
def update_user_codex_labs_access(
    ...
    if request.enabled and not ENABLE_CODEX_LABS:
        raise HTTPException(
            status_code=400,
            detail="Codex Labs is globally disabled",
        )

    # When globally disabled, always persist False to avoid stale grants.
    target_enabled = request.enabled if ENABLE_CODEX_LABS else False

    if user_to_update.enable_codex_labs == target_enabled:
        return

    user_to_update.enable_codex_labs = target_enabled
    db_session.commit()
