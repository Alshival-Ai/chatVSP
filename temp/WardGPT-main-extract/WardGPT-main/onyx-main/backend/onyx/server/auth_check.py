...
PUBLIC_ENDPOINT_SPECS = [
    ...
    # codex labs websocket terminal stream — access enforced via short-lived
    # ticket issued from authenticated API endpoints
    ("/codex-labs/terminal/ws", set()),
]
...
def is_route_in_spec_list(
    ...
    if route_methods is not None and (route_path, route_methods) in public_endpoint_specs:
        return True

    # WebSocket routes do not expose HTTP methods; treat exact path matches
    # with an empty method set as explicit public specs.
    if route_methods is None and (route_path, set()) in public_endpoint_specs:
        return True
    ...
    for endpoint_spec in public_endpoint_specs:
        ...
        if prefixed_path == route_path and route_methods == methods:
            return True
        if prefixed_path == route_path and route_methods is None and methods == set():
            return True
