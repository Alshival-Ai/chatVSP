...
def _build_index_filters(
    ...
    user_document_set_filter = [
        doc_set.strip()
        for doc_set in (base_filters.document_set or [])
        if doc_set and doc_set.strip()
    ]
    persona_document_set_filter = (
        [
            doc_set.strip()
            for doc_set in (persona_document_sets or [])
            if doc_set and doc_set.strip()
        ]
        if persona_document_sets is not None
        else None
    )
    # Empty user-provided document-set filters should not disable persona scoping.
    document_set_filter = user_document_set_filter or persona_document_set_filter
