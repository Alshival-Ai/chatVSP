...
from onyx.configs.app_configs import LOCAL_MCP_SERVER_OWNER_EMAIL
...
def ensure_local_mcp_server(db_session: Session) -> None:
    """Create/update a default local MCP server record for MCP Actions."""
    if not MCP_SERVER_ENABLED or not AUTO_REGISTER_LOCAL_MCP_SERVER:
        return

    existing_by_name = db_session.scalar(
        select(MCPServer).where(MCPServer.name == LOCAL_MCP_SERVER_ACTION_NAME)
    )
    if existing_by_name:
        if existing_by_name.server_url != LOCAL_MCP_SERVER_URL:
            update_mcp_server__no_commit(
                server_id=existing_by_name.id,
                db_session=db_session,
                server_url=LOCAL_MCP_SERVER_URL,
            )
            db_session.commit()
            logger.notice(
                "Updated local MCP server URL for '%s' to %s",
                LOCAL_MCP_SERVER_ACTION_NAME,
                LOCAL_MCP_SERVER_URL,
            )
        return

    existing_by_url = db_session.scalar(
        select(MCPServer).where(MCPServer.server_url == LOCAL_MCP_SERVER_URL)
    )
    if existing_by_url:
        update_mcp_server__no_commit(
            server_id=existing_by_url.id,
            db_session=db_session,
            name=LOCAL_MCP_SERVER_ACTION_NAME,
            description=(
                "Auto-registered local MCP endpoint for this WardGPT deployment."
            ),
        )
        db_session.commit()
        logger.notice(
            "Reused existing MCP server URL and renamed it to '%s'.",
            LOCAL_MCP_SERVER_ACTION_NAME,
        )
        return

    create_mcp_server__no_commit(
        owner_email=LOCAL_MCP_SERVER_OWNER_EMAIL,
        name=LOCAL_MCP_SERVER_ACTION_NAME,
        description="Auto-registered local MCP endpoint for this WardGPT deployment.",
        server_url=LOCAL_MCP_SERVER_URL,
    ...
    db_session.commit()
    logger.notice(
        "Auto-registered MCP server '%s' at %s",
        LOCAL_MCP_SERVER_ACTION_NAME,
        LOCAL_MCP_SERVER_URL,
    )
...
def setup_postgres(db_session: Session) -> None:
    ...
    ensure_local_mcp_server(db_session)
    rotate_local_mcp_server_api_key(db_session)
