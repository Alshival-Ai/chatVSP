...
def kml_to_text(file: IO[Any]) -> str:
    file.seek(0)
    return _decode_text_bytes(file.read())


def kmz_to_text(file: IO[Any]) -> str:
    file.seek(0)
    with zipfile.ZipFile(file, "r") as zf:
        kml_name = "doc.kml" if "doc.kml" in zf.namelist() else None
        if not kml_name:
        ...
        if not kml_name:
            return ""
        with zf.open(kml_name, "r") as kml_file:
            return _decode_text_bytes(kml_file.read())
...
def _extract_text_and_images(
    ...
    try:
        ...
        if extension == ".kml":
            return ExtractionResult(
                text_content=kml_to_text(file), embedded_images=[], metadata={}
            )

        if extension == ".kmz":
            return ExtractionResult(
                text_content=kmz_to_text(file), embedded_images=[], metadata={}
            )
