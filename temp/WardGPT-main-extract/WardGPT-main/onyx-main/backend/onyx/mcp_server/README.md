...
The server provides tools for searching and retrieving information:
...
4. `google_places_search_text`
Search Google Places with a text query and return normalized candidates with coordinates.

5. `google_places_geocode_address`
Resolve an address/site string to a best coordinate and return alternatives.

6. `google_places_get_place_details`
Fetch canonical details for a specific Google Place ID.

7. `google_places_search_nearby`
Find nearby places around a coordinate to validate anchors/map context.
...
**Google Places Tools:**
- `GOOGLE_API_KEY`: Required to enable the Google Places MCP tools.
