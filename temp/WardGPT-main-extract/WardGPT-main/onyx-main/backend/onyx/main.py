...
from onyx.auth.oauth_clients import MicrosoftOpenID
...
from onyx.configs.app_configs import LOCAL_MCP_TOOL_SYNC_ON_STARTUP
from onyx.configs.app_configs import LOCAL_MCP_TOOL_SYNC_STARTUP_DELAY_SECONDS
...
from onyx.db.local_mcp_server import sync_local_mcp_server_tools
...
from onyx.server.features.codex_labs.api import ws_router as codex_labs_ws_router
...
async def _delayed_local_mcp_tool_sync() -> None:
    """Best-effort delayed MCP tool sync to avoid API/MCP startup race."""
    try:
        if LOCAL_MCP_TOOL_SYNC_STARTUP_DELAY_SECONDS > 0:
            await asyncio.sleep(LOCAL_MCP_TOOL_SYNC_STARTUP_DELAY_SECONDS)

        with get_session_with_current_tenant() as db_session:
            changed = sync_local_mcp_server_tools(db_session)
            if changed:
                logger.notice("Startup local MCP tool sync completed with DB changes.")
            else:
                logger.debug("Startup local MCP tool sync completed with no DB changes.")
    except Exception:
        logger.exception("Startup local MCP tool sync failed.")
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:  # noqa: ARG001
    ...
    startup_mcp_sync_task: asyncio.Task[None] | None = None
    if (
        not MULTI_TENANT
        and LOCAL_MCP_TOOL_SYNC_ON_STARTUP
        and CURRENT_TENANT_ID_CONTEXTVAR.get() == POSTGRES_DEFAULT_SCHEMA
    ):
        startup_mcp_sync_task = asyncio.create_task(_delayed_local_mcp_tool_sync())

    yield

    if startup_mcp_sync_task and not startup_mcp_sync_task.done():
        startup_mcp_sync_task.cancel()
        try:
            await startup_mcp_sync_task
...
def get_application(lifespan_override: Lifespan | None = None) -> FastAPI:
    ...
    include_router_with_global_prefix_prepended(application, codex_labs_ws_router)
