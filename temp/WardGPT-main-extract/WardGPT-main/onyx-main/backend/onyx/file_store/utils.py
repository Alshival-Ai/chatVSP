...
def load_chat_files_by_ids(file_ids: list[str]) -> list[InMemoryChatFile]:
    """Load chat-attached files directly from the file store."""
    return cast(
        list[InMemoryChatFile],
        run_functions_tuples_in_parallel(
            [(load_chat_file_by_id, (file_id,)) for file_id in file_ids]
        ),
    )
...
def verify_user_files(
    ...
    # Extract user_file_ids, chat file_ids, and project file_ids from the file descriptors
    user_file_ids = []
    chat_file_ids = []
    project_file_ids = []
    ...
    for file_descriptor in user_files:
        ...
        if file_descriptor.get("user_file_id"):
            try:
                ...
                continue
        elif file_descriptor.get("is_chat_file"):
            if file_descriptor.get("id"):
                chat_file_ids.append(file_descriptor["id"])
        else:
    ...
    if chat_file_ids:
        file_store = get_default_file_store()
        for chat_file_id in chat_file_ids:
            if not file_store.read_file_record(chat_file_id):
                raise ValueError(f"Chat file {chat_file_id} not found")
