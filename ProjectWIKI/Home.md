...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - desktop `Text Editor` now excludes Monaco's hidden `.inputarea` from the app's global textarea styling so the editor's keyboard-capture input no longer shows up as a stray resizable box above the real editor surface
