...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - the bottom taskbar now uses theme-aware glass and icon contrast, so light mode keeps the icons readable even when a maximized light window sits behind the dock
    ...
    - desktop `Text Editor` now keeps its sidebar, compact toolbar, tab rail, editor canvas, and save modal aligned to the active light/dark theme instead of mixing light and dark surface treatments
    - desktop `Text Editor` now excludes Monaco's hidden `.inputarea` from the app's global textarea styling and also reapplies Monaco's hidden-input positioning with an explicit global override, so the editor's keyboard-capture textarea no longer shows up as a stray box above the real editor surface
    ...
    - text files such as `.txt`, `.json`, `.md`, `.py`, and similar now open into the focused desktop editor window as tabs rather than using a separate preview-window editor mode, and the file explorer/tree context menus now expose `Open in Text Editor` explicitly for editable files including HTML
