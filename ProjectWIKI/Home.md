...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - desktop `Text Editor` now uses Monaco's modern `editContext` path, while the Neural Labs Monaco host only hides the legacy hidden textarea input and suppresses the browser focus outline on Monaco's edit-context layer so stray browser boxes do not leak into the visible layout
