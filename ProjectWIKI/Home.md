...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - desktop `Text Editor` now loads Monaco's official editor stylesheet at the app root and runs Monaco with `useShadowDOM: false` in Neural Labs, removing the ad-hoc host CSS hacks so caret/input behavior and hidden editor internals render correctly without stray browser input boxes
