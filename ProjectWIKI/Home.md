...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - desktop `Text Editor` now gives the Monaco host its own scoped overrides for Monaco's internal keyboard-capture elements (`.inputarea`, `.ime-text-area`, and `.native-edit-context`), so those hidden edit surfaces stay invisible instead of showing up as stray boxes above the real editor surface
