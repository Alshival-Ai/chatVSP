...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - desktop `Text Editor` now gives the Monaco host its own scoped textarea override so any keyboard-capture textarea Monaco mounts inside that editor stays hidden instead of showing up as a stray resizable box above the real editor surface
