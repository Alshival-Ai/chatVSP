...
- Current live scope includes:
  ...
  - modular web UI with:
    ...
    - desktop `Text Editor` now disables Monaco's browser `editContext` path for this app and scopes a hidden-input override to the Monaco host, so the editor uses the stable textarea input path without leaking stray browser input boxes into the visible layout
