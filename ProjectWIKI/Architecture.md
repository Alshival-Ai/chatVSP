...
- Frontend window model:
  ...
  - the desktop text editor wraps Monaco in a dedicated host class and uses Monaco's modern `editContext` mode; the host CSS only hides the legacy hidden textarea path and suppresses browser focus outline styling on Monaco's edit-context layer so input plumbing does not render as visible UI
