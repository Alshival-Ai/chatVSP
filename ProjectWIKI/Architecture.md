...
- Frontend window model:
  ...
  - the desktop text editor now disables Monaco's `editContext` mode and wraps Monaco in a dedicated host class with a scoped hidden-input override, so the editor uses the stable textarea input path without leaking browser input controls into the visible editor layout
