...
- Frontend window model:
  ...
  - the app's global textarea styling is explicitly scoped away from Monaco's hidden `.inputarea`, and the app reapplies Monaco's hidden-input layout with a high-specificity global override so the keyboard-capture textarea remains invisible instead of leaking into the visible editor layout
