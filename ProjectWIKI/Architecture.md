...
- Frontend window model:
  ...
  - the desktop text editor now wraps Monaco in a dedicated host class and scopes a textarea override to that host, so any keyboard-capture textarea Monaco mounts there remains invisible instead of leaking into the visible editor layout
