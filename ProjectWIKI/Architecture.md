...
- Frontend window model:
  ...
  - the desktop text editor now wraps Monaco in a dedicated host class and scopes overrides for Monaco's internal edit surfaces (`.inputarea`, `.ime-text-area`, `.native-edit-context`) to that host, so hidden keyboard-capture elements remain invisible instead of leaking into the visible editor layout
