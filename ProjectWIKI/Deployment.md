...
- browser UI for:
  ...
  - the desktop text editor now keeps Monaco's `editContext` mode enabled and scopes the Neural Labs CSS so only the legacy hidden textarea path is hidden while the modern edit-context layer has its browser outline suppressed, which removes the stray browser box without breaking caret positioning
