...
- browser UI for:
  ...
  - the desktop text editor now disables Monaco's `editContext` mode for this app and scopes the remaining hidden-input override to the Monaco host, which removes the stray browser input boxes while preserving correct caret positioning in the editor surface
