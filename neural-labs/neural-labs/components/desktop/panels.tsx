...
async function parseSpreadsheetPreview(
  ...
  const parsedRows = rows.map((rowNode) => {
    ...
    Array.from(rowNode.getElementsByTagName("c")).forEach(
      (cellNode, fallbackIndex) => {
        const columnIndex =
          getCellColumnIndex(cellNode.getAttribute("r")) ?? fallbackIndex;
  ...
  const visibleRows =
    lastRowIndex >= 0 ? parsedRows.slice(0, lastRowIndex + 1) : [];
...
function SpreadsheetPreview({
  ...
  const [preview, setPreview] = useState<SpreadsheetPreviewState>({
    status: "loading",
  });
  ...
  if (preview.status === "loading") {
    return (
      <div className="nl-spreadsheet-preview__state">
...
export function PreviewPanel({
  ...
  if (
    ...
    return (
      <iframe className="nl-preview-frame" src={fileUrl} title={entry.name} />
    );
  }

  return (
    <pre className="nl-preview-text">{fileText ?? "Loading preview..."}</pre>
  );
}
  ...
  return (
    <div className="nl-panel nl-neura-app">
      <div
        className={cn(
          "nl-neura-layout",
          !sidebarOpen && "nl-neura-layout--collapsed",
        )}
      >
        {sidebarOpen ? (
          ...
          <header className="nl-neura-header">
            <div className="nl-neura-header__title">
              ...
              <div className="nl-neura-header__meta">
                <strong>
                  {activeConversation?.summary.title ?? assistantName}
                </strong>
                <span>{providerLabel}</span>
              </div>
            </div>
          ...
          >
            {visibleMessages.length ? (
              ...
              <div className="nl-neura-empty-state">
                ...
                <span>
                  Ask Neura about your workspace, files, code, or project
                  context.
                </span>
              </div>
            )}
...
export function SettingsPanel({
  ...
  async function handleCreatePasswordReset(userId: string) {
    ...
    try {
      ...
      setResets((current) => [
        ...
        ...current.filter(
          (entry) => entry.userId !== reset.userId || entry.usedAt,
        ),
      ]);
  ...
  async function handleGenerateTemporaryPassword(userId: string) {
    ...
    setLatestTemporaryPassword("");
    try {
      const result = await setUserPasswordRequest(userId, {
  ...
  async function handleDeleteUser(
    user: AuthAdminUserRecord,
    deleteWorkspace: boolean,
  ) {
    const confirmation = deleteWorkspace
      ? window.prompt(
          `Type DELETE WORKSPACE to delete ${user.email} and remove workspace data.`,
        )
      : window.prompt(
          `Type DELETE to delete ${user.email}. Workspace data will be preserved.`,
        );
    ...
    try {
      ...
      setAdminStatusText(
        deleteWorkspace ? "User and workspace deleted." : "User deleted.",
      );
    } catch (error) {
      setAdminStatusText(
  ...
  async function handleRevokePasswordReset(resetId: string) {
    ...
    try {
      ...
      setResets((current) =>
        current.map((reset) =>
          reset.id === resetId
            ? { ...reset, revokedAt: new Date().toISOString() }
            : reset,
        ),
      );
      ...
      setAdminStatusText(
        error instanceof Error ? error.message : "Unable to revoke reset",
      );
    } finally {
                ...
                {customBackgroundUrl ? (
                  <>
                    ...
                    <div className="nl-toolbar-actions">
                      ...
                      <Button
                        variant="danger"
                        onClick={onDeleteCustomBackground}
                      >
                        Delete
                      </Button>
                    </div>
            ...
            <div className="nl-settings-section-stack">
              <div className="nl-panel__toolbar nl-panel__toolbar--stacked">
                <div>
                  <h3>Providers</h3>
                  <p className="nl-muted-copy">
                    Configure the model endpoints Neural Labs can use and choose
                    the default.
                  </p>
                </div>
              </div>
              ...
              <div className="nl-settings-provider-layout">
                  ...
                  <div className="nl-settings-card">
                    ...
                    <div className="nl-list">
                      {providers.length ? (
                        ...
                        <div className="nl-empty-state">
                          No providers configured yet. Start with a template on
                          the right.
                        </div>
                      )}
                    </div>
                ...
                <div className="nl-settings-provider-detail">
                  <div className="nl-settings-card">
                        ...
                        <>
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              setStatus(
                                await onTestProvider(selectedProviderId),
                              );
                            }}
                          >
                            Test
                    ...
                    {status ? (
                      <div className="nl-status-note">{status}</div>
                    ) : null}
                  </div>
                </div>
              </div>
              ...
              <div className="nl-settings-card">
                ...
                <div className="nl-settings-avatar-card">
                  ...
                  <div className="nl-settings-avatar-card__copy">
                    <strong>
                      {avatarUrl ? "Current avatar" : "No avatar uploaded"}
                    </strong>
                    <span>
                      {avatarUrl
              ...
              <div className="nl-settings-card">
                <div className="nl-settings-card__header">
                  <div>
                    ...
                    <p className="nl-muted-copy">
                      This account maps to the persistent Neural Labs workspace
                      and container.
                    </p>
                  </div>
                </div>
          ...
          {activeSection === "admin" && viewer.role === "admin" ? (
            <div className="nl-settings-section-stack">
              ...
              <div className="nl-settings-card">
                ...
                >
                  ...
                  <Field label="Role">
                    <Select
                      value={userRole}
                      onChange={(event) =>
                        setUserRole(event.target.value as AuthRole)
                      }
                    >
                  ...
                  <label className="nl-inline-checkbox">
                    <input
                      ...
                      onChange={(event) =>
                        setShouldGeneratePassword(event.target.checked)
                      }
                    />
                ...
                {latestResetUrl ? (
                  <div className="nl-auth-callout">
                    ...
                    <Button
                      variant="ghost"
                      onClick={() =>
                        void navigator.clipboard.writeText(latestResetUrl)
                      }
                    >
                      Copy Link
                    </Button>
                ...
                <div className="nl-admin-user-list">
                  {isLoadingInvites ? (
                    ...
                    <div className="nl-auth-table__empty">
                    ...
                    users.map((user) => (
                      <div key={user.id} className="nl-admin-user-row">
                        <div className="nl-admin-user-row__meta">
                          ...
                          <span>
                            <strong>
                              {user.disabledAt ? "Suspended" : "Active"}
                            </strong>
                            <small>Status</small>
                          </span>
                          <span>
                        ...
                        <div className="nl-admin-user-row__actions">
                          ...
                          <Button
                            ...
                            disabled={workingUserId === user.id}
                            onClick={() =>
                              void handleCreatePasswordReset(user.id)
                            }
                          >
                            Reset Link
                          </Button>
                          <Button
                            ...
                            disabled={workingUserId === user.id}
                            onClick={() =>
                              void handleGenerateTemporaryPassword(user.id)
                            }
                          >
                            Temp Password
                          </Button>
                          ...
                          <Button
                            variant="danger"
                            disabled={
                              workingUserId === user.id || user.id === viewer.id
                            }
                        ...
                        <span>
                          {!reset.usedAt && !reset.revokedAt ? (
                            <Button
                              ...
                              onClick={() =>
                                void handleRevokePasswordReset(reset.id)
                              }
                            >
                              {revokingResetId === reset.id
                                ? "Revoking..."
                                : "Revoke"}
                            </Button>
                          ) : null}
              ...
              <div className="nl-settings-card">
                ...
                <form
                  className="nl-settings-inline-form"
...
export const DOCK_APPS = [
  ...
  },
  {
    kind: "vscode",
    label: "VS Code",
    iconSrc: withBasePath("/apps/vscode.png"),
    accent: "#2d9bf0",
  },
