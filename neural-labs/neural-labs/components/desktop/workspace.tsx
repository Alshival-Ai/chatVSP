...
export function NeuralLabsWorkspace({
    ...
    async function bootstrap() {
      try {
        const settingsPromise = initialSettings
          ...
          : fetchSettings();
        const [nextSettings, nextListing, config, conversationIndex] =
          await Promise.all([
            settingsPromise,
            listFiles(""),
            fetchNeuraConfig(),
        ...
        setNotice(
          error instanceof Error ? error.message : "Unable to load Neural Labs",
        );
      }
    }
  ...
  async function openPreview(entry: FileEntry) {
    const existing = windows.find(
      (window) =>
        window.kind === "preview" &&
        previewWindows[window.id]?.entry.path === entry.path,
    );
    if (existing) {
      focusWindow(existing.id);
      return;
        ...
        layout: {
          ...
          tabs: layout.tabs.map((candidate) => {
            ...
            return {
              ...
              splitMode:
                remainingPanes.length > 1 ? candidate.splitMode : "none",
            };
          }),
        },
      };
      ...
      const nextActiveTabId =
        state.activeTabId === tabId
          ? (remainingTabs[
              Math.max(
                0,
                state.tabs.findIndex((tab) => tab.tabId === tabId) - 1,
              )
  ...
  async function closeWindow(windowId: string) {
    ...
    if (target.kind === "terminal") {
      ...
      await Promise.allSettled(
        sessionIds.map((sessionId) => closeTerminalSession(sessionId)),
      );
    }
      ...
      <div className="nl-desktop" style={{ backgroundImage: backgroundStyle }}>
        ...
        <main ref={workspaceRef} className="nl-workspace">
          ...
          {windows.map((window) => {
            const active =
              window.zIndex ===
              Math.max(...windows.map((entry) => entry.zIndex));
            ...
            return (
              ...
              >
                {window.kind === "files" ? (
                  <FileExplorerPanel
                    ...
                    onSetAsBackground={async (entry) => {
                      const result = await setDesktopBackgroundFromFile(
                        entry.path,
                      );
                ...
                {window.kind === "editor" ? (
                  <TextEditorPanel
                    windowState={
                      editorWindows[window.id] ??
                      createDefaultEditorWindowState()
                    }
                    ...
                    onChangeTabContent={(tabId, content) =>
                      updateEditorWindowState(window.id, (state) => ({
                        ...
                        tabs: state.tabs.map((tab) =>
                          tab.tabId === tabId
                            ? { ...tab, content, errorMessage: null }
                            : tab,
                        ),
                      }))
                    }
                    onSaveTab={(tabId) => saveEditorTab(window.id, tabId)}
                    onSaveTabAs={(tabId, path) =>
                      saveEditorTabAs(window.id, tabId, path)
                    }
                ...
                {window.kind === "terminal" && terminalWindows[window.id] ? (
                  <TerminalPanel
                    ...
                    onDuplicateTab={(tabId) =>
                      duplicateTerminalTab(window.id, tabId)
                    }
                        ...
                        layout: state.layout
                          ? {
                              ...
                              activeTabId: tabs.some(
                                (tab) =>
                                  tab.tabId === state.layout?.activeTabId,
                              )
                ...
                {window.kind === "neura" ? (
                  <NeuraPanel
                    ...
                    defaultModel={
                      neuraConfig?.defaultModel ?? "No default model"
                    }
                    defaultProviderName={
                      neuraConfig?.defaultProviderName ?? null
                    }
                  ...
                  <SettingsPanel
                    ...
                    onUploadCustomBackground={async (file) => {
                      if (!file.type.startsWith("image/")) {
                        throw new Error(
                          "Choose an image file for the desktop background",
                        );
                      }
                    ...
                    onUploadAvatar={async (file) => {
                      ...
                      if (
                        ...
                        await deletePath(previousAvatarPath).catch(
                          () => undefined,
                        );
                      }
                      await refreshListing(listing?.path ?? "");
