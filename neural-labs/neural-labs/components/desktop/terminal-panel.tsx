...
function TerminalPaneSurface({
  ...
  const publishMeta = useCallback((meta: Partial<TerminalPaneMeta>) => {
    const nextMeta = {
      cols:
        meta.cols ?? terminalRef.current?.cols ?? terminalSizeRef.current.cols,
      rows:
        meta.rows ?? terminalRef.current?.rows ?? terminalSizeRef.current.rows,
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          !/terminal session not found|terminal_id is required/i.test(message)
        ) {
          throw error;
        }
  ...
  return (
    ...
    >
      <div
        className="nl-terminal-app__pane-toolbar"
        aria-label="Terminal pane tools"
      >
        <span
          className={cn(
      ...
      <div
        className="nl-terminal-app__quick-actions"
...
export function TerminalPanel({
  ...
  const [paneMetaById, setPaneMetaById] = useState<
    Record<string, TerminalPaneMeta>
  >({});
  const [splitRatioByTabId, setSplitRatioByTabId] = useState<
  ...
  const activeTab = useMemo(() => {
    ...
    return (
      layout.tabs.find((tab) => tab.tabId === layout.activeTabId) ??
      layout.tabs[0] ??
      null
    );
  }, [layout]);
    ...
    <div className="nl-panel nl-panel--terminal nl-terminal-app">
      <div className="nl-terminal-app__header">
        <div className="nl-terminal-app__topbar">
          ...
          <div
            className="nl-terminal-app__actions"
      ...
      <div className="nl-terminal-app__body">
        {isInitializing ? (
          ...
          <div
            ...
            onMouseDown={() =>
              activePane && onSetActivePane(activeTab.tabId, activePane.paneId)
            }
          >
            {activePane ? (
              <TerminalPaneSurface
                ...
                onFocus={() =>
                  onSetActivePane(activeTab.tabId, activePane.paneId)
                }
                onMetaChange={(meta) =>
                  handlePaneMetaChange(activePane.paneId, meta)
                }
              ...
              return (
                <Fragment key={pane.paneId}>
                  ...
                  <div
                    ...
                    onMouseDown={() =>
                      onSetActivePane(activeTab.tabId, pane.paneId)
                    }
                  >
                    ...
                    <div className="nl-terminal-app__pane-body">
                      <TerminalPaneSurface
                        ...
                        onMetaChange={(meta) =>
                          handlePaneMetaChange(pane.paneId, meta)
                        }
                        onSessionRecovered={(nextSessionId) =>
                          onRecoverPaneSession(
                            activeTab.tabId,
      ...
      <div className="nl-terminal-app__footer">
        ...
        <span>
          ...
          {activeTab
            ? ` / ${activeTab.panes.length} ${activeTab.panes.length === 1 ? "pane" : "panes"}`
            : ""}
        </span>
      </div>
    </div>
