...
function encodePassword(password: string): string {
  ...
  const derived = scryptSync(password, salt, PASSWORD_HASH_BYTES).toString(
    "hex",
  );
...
function toPasswordResetRecord(
  row: PasswordResetRow & { email: string },
): AuthPasswordResetRecord {
  return {
    id: row.id,
    userId: row.user_id,
...
function assertCanRemoveActiveAdmin(
  actor: AuthViewer,
  target: UserRow,
  action: string,
): void {
...
function findPasswordResetByToken(
  token: string,
): (PasswordResetRow & { email: string }) | null {
  const row = getDb()
    .prepare(
      `SELECT
         pr.id,
...
export function canBootstrapAdmin(): boolean {
  return (
    !hasAnyUsers() &&
    Boolean(getInitialAdminEmail() && getInitialAdminPassword())
  );
}
...
export function getSessionContextFromRequest(
  request: Request,
...
export function getViewerFromCookieHeader(
  cookieHeader: string | null,
): AuthViewer | null {
...
export function getViewerBySessionToken(
  sessionToken: string,
): AuthViewer | null {
  const session = readSessionFromToken(sessionToken);
  if (!session) {
...
export function loginWithTrustedUser(
  email: string,
  role: AuthRole = "user",
): {
  viewer: AuthViewer;
  ...
  if (!user) {
    user = createUser(normalizedEmail, generateTemporaryPassword(), role);
  } else if (user.role !== role || user.disabled_at) {
    db.prepare(
      `UPDATE users
       SET role = ?, disabled_at = NULL, updated_at = ?
       WHERE id = ?`,
  ...
  if (!user) {
    throw new AuthError("Unable to provision trusted user", 500);
  }
...
export function bootstrapAdminAccount(
  ...
  if (normalizeEmail(email) !== initialAdminEmail) {
    throw new AuthError(
      "Email does not match the configured initial admin",
      403,
    );
  }
  if (password !== initialAdminPassword) {
    throw new AuthError(
      "Password does not match the configured initial admin",
      403,
    );
  }
...
export function createInvite(
  ...
  const token = generateOpaqueToken();
  const expiresAt = new Date(
    now.getTime() + INVITE_TTL_SECONDS * 1000,
  ).toISOString();
...
export function updateUserAsAdmin(
  ...
  if (
    (nextRole !== "admin" || nextDisabledAt) &&
    user.role === "admin" &&
    !user.disabled_at
  ) {
    assertCanRemoveActiveAdmin(
      actor,
      user,
      nextRole !== "admin" ? "demote" : "suspend",
    );
  }
...
export function listPasswordResets(
  actor: AuthViewer,
): AuthPasswordResetRecord[] {
  if (actor.role !== "admin") {
    throw new AuthError("Admin access required", 403);
...
export function getPasswordResetPreview(
  token: string,
): AuthPasswordResetRecord {
...
export function acceptPasswordReset(
  token: string,
  password: string,
): {
  viewer: AuthViewer;
  ...
  try {
    ...
    ).run(encodePassword(password), new Date().toISOString(), user.id);
