...
import { type NeuralLabsFileEntry } from "@/app/neural-labs/types";
...
const SHOW_HIDDEN_STORAGE_KEY = "neural-labs-show-hidden-files-v1";

interface NeuralLabsFileTreeProps {
  entriesByPath: Record<string, NeuralLabsFileEntry[]>;
...
function isHiddenEntry(entry: NeuralLabsFileEntry): boolean {
...
function TreeRow({
  ...
  onDragStartEntry: (entry: NeuralLabsFileEntry) => void;
  onDragEndEntry: () => void;
  onDragOverDirectory: (
...
export default function NeuralLabsFileTree({
  entriesByPath,
  expandedPaths,
  loadingPaths,
  selectedPath,
  ...
  const entryByPath = useMemo(() => {
    const result = new Map<string, NeuralLabsFileEntry>();
